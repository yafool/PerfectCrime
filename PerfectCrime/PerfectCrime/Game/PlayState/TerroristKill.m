...
#import "TerroristKill.h"
#import "VoteBoxView.h"
#import "PlayersTalk.h"

static NSString *Wate2Do          =@"杀一个";

@interface TerroristKill ()
//倒计时
...
@implementation TerroristKill
-(void)OnPlaying{
    GameConfig * config = [GameInfoManager sharedGameInfoManager].gameConfig;
    self.countdown = config.murderNightKillTime;
    
    [[GameHandler shareInstance]terroristKill];
    
    dispatch_async(dispatch_get_main_queue(), ^{
        [VoteBoxView showWithVoteType:VoteBoxType_4Terrorist andVoteActionBlock:^(VoteBoxType type, PlayerInfo *player){
            ...
            // player 出局
            [[GameHandler shareInstance]vote2Kill:player];
