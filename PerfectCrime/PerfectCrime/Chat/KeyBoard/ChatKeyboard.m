...
static CGFloat keyboardWidth         = 0;
...
+ (ChatKeyboard*)initWithKeyboardWidth:(CGFloat)width{
    keyboardWidth = width;
    ChatKeyboard *chatKeyboard = [[ChatKeyboard alloc]init];
    return chatKeyboard;
}
//录音工具
...
- (UIView *)emotionBottonBar
{
    if (!_emotionBottonBar) {
        ...
        sendButton.frame = Frame(keyboardWidth/2 - 30, 5, 60, 30);
...
- (UIScrollView *)emotionScrollView
{
    if (!_emotionScrollView) {
        ...
        //中间间距
        CGFloat midMargin = (keyboardWidth - columnMaxCount*widthHeight - 2*lrMargin)/(columnMaxCount - 1);
        //计算一共多少页表情
        NSInteger pageCount = self.emotionDict.count / emotionMaxCount + (self.emotionDict.count %emotionMaxCount > 0 ? 1 : 0);
        //滑动范围
        _emotionScrollView.contentSize = CGSizeMake(pageCount *keyboardWidth, 0);
...
- (UIView *)messageBar
{
    if (!_messageBar) {
        ...
        [_messageBar addSubview:self.keyboardButton];
...
- (UIButton *)keyboardButton
{
    if (!_keyboardButton) {
        ...
        [_keyboardButton setImage:LoadImage(@"键盘") forState:UIControlStateNormal];
...
- (void)configUIFrame
{
    self.messageBar.frame = Frame(0, 0, keyboardWidth, defaultMsgBarHeight);  //消息栏
    // TO-DO: 目前只支持 文字和表情输入
    self.audioButton.frame = Frame(10, (Height(self.messageBar.frame) - 30)*0.5, 30, 30); //语音按钮
    self.keyboardButton.frame = Frame(10, (Height(self.messageBar.frame) - 30)*0.5, 30, 30); //键盘按钮
    self.audioLpButton.frame = Frame(MaxX(self.keyboardButton.frame)+15,(Height(self.messageBar.frame)-defaultInputHeight)*0.5, keyboardWidth - 108, defaultInputHeight); //长按录音按钮
    ...
    self.keyBoardContainer.frame = Frame(0,Height(self.messageBar.frame)+1, keyboardWidth,CTKEYBOARD_DEFAULTHEIGHT - Height(self.messageBar.frame)); //自定义键盘容器
    ...
    //表情容器部分
    self.emotionScrollView.frame =Frame(0,0, keyboardWidth, Height(self.facesKeyboard.frame)-40); //表情滚动容器
    for (NSInteger index = 0; index < self.emotionScrollView.subviews.count; index ++) { //emotion容器
        ...
        emotionView.frame = Frame(index *keyboardWidth, 0, keyboardWidth, Height(self.emotionScrollView.frame));
    }
    //页码
    ...
    self.emotionPgControl.frame = Frame((keyboardWidth - controlSize.width)*0.5,Height(self.emotionScrollView.frame)-controlSize.height, controlSize.width, controlSize.height); // pageControl
    self.emotionBottonBar.frame = Frame(0,MaxY(self.emotionScrollView.frame), keyboardWidth, 40); //底部操作栏  固定 40高度
}
...
- (void)systemKeyboardWillShow:(NSNotification *)note
{
    ...
    //将自定义键盘跟随位移
    [self customKeyboardMove:VIEW_HEIGHT - systemKbHeight - Height(self.messageBar.frame)];
    // 禁用 slide menu
...
- (void)switchFaceKeyboard:(UIButton *)swtFaceButton
{
    ...
    if (swtFaceButton.selected) {
        ...
        //自定义键盘位移
        [self customKeyboardMove:VIEW_HEIGHT - Height(self.frame)];
        // 禁用 slide menu
        AppDelegate * appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
        [appDelegate disableSlidMenu];
    }else{
...
- (void)audioButtonClick:(UIButton *)audioButton
{
    ...
    if (audioButton.selected) {
        ...
        [self customKeyboardMove:VIEW_HEIGHT - defaultMsgBarHeight]; //默认高度 输入栏 49
    }else{
...
{
    keyboardButton.selected = !keyboardButton.selected;
    //重置其他按钮selected
    self.swtFaceButton.selected = NO;
    self.swtHandleButton.selected = NO;
    ...
    keyboardButton.selected = !_msgTextView.isFirstResponder;
}
#pragma mark - 自定义键盘位移变化
- (void)customKeyboardMove:(CGFloat)customKbY
{
    [UIView animateWithDuration:0.25 animations:^{
        self.frame = Frame(0,customKbY, Width(self.frame), Height(self.frame));
    }];
}
...
{
    self.messageBar.frame = Frame(0, 0, keyboardWidth, msgViewHeight +MinY(self.msgTextView.frame)*2);
    ...
    self.keyBoardContainer.frame = Frame(0, MaxY(self.messageBar.frame), keyboardWidth, Height(self.keyBoardContainer.frame));
    self.frame = Frame(0,VIEW_HEIGHT - keyboardHeight-Height(self.messageBar.frame), Width(self.frame),Height(self.keyBoardContainer.frame) + Height(self.messageBar.frame));
}
...
{
    self.emotionPgControl.currentPage = scrollView.contentOffset.x / keyboardWidth;
}
...
- (void)keyboardResignFirstResponder:(NSNotification *)note
{
    ...
    //按钮初始化刷新
    [self reloadSwitchButtons];
    [self customKeyboardMove:VIEW_HEIGHT - Height(self.messageBar.frame)];
