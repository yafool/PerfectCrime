...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.backgroundColor = [UIColor clearColor];//UIMainBackColor;
...
- (void)setTextModel:(ChatModel *)textModel
{
    ...
    if (textModel.shouldShowTime) {
        ...
        CGSize timeTextSize  = [self.timeLabel sizeThatFits:CGSizeMake(self.frame.size.width, 20)];
        self.timeLabel.frame = Frame(5,(20 - timeTextSize.height)*0.5, timeTextSize.width, timeTextSize.height);
        self.timeContainer.frame = Frame((self.frame.size.width - timeTextSize.width-10)*0.5, 15,timeTextSize.width + 10, 20);
    }else{
        self.timeContainer.hidden = YES;
    }
    //处理转圈
    textModel.isSending &&textModel.byMyself ? [self.activiView startAnimating] : [self.activiView stopAnimating];
    //处理红叹号
    self.failureButton.hidden = textModel.isSend || textModel.isSending || !textModel.byMyself;
    //处理昵称显示
    self.nickNameLabel.hidden = textModel.byMyself;
    //赋值
    [self setContent];
    //设置frame
    [self setFrame];
...
- (void)setContent
{
    [self.iconView downloadImage:_textModel.picUrl placeholder:@"defaultHead"];
    
    if (![CustomStringFunc isBlankString:_textModel.picFrame]) {
        CGRect frame = self.iconView.frame;
        frame.origin.x = 0;
        frame.origin.y = 0;
        UIImageView *photoFrame = [[UIImageView alloc]initWithFrame:frame];
        photoFrame.backgroundColor = [UIColor clearColor];
        photoFrame.image = [UIImage imageNamed:_textModel.picFrame];
        [self.iconView addSubview:photoFrame];
    }
    
    UIImage *backImage = nil;
    //我方
    if (_textModel.byMyself) {
...
- (void)setFrame
{
    CGSize size = [_coreLabel sizeThatFits:CGSizeMake(self.frame.size.width - 145, MAXFLOAT)];
    if (_textModel.byMyself) {
        //文本宽度JYScreen_Width - 145
        //我方头像
        self.iconView.frame = Frame(self.frame.size.width - 65, MaxY(self.timeContainer.frame)+15, 50, 50);
        //我方文本label
        self.coreLabel.frame = Frame(10, 10,size.width, size.height);
        //我方背景气泡
        self.backButton.frame = Frame(self.frame.size.width - 100 - Width(self.coreLabel.frame), MinY(self.iconView.frame)+5, Width(self.coreLabel.frame)+30, Height(self.coreLabel.frame)+20);
