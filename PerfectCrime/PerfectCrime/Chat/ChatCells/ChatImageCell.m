...
- (void)setImageModel:(ChatModel *)imageModel
{
    ...
    self.failureButton.hidden    = !imageModel.byMyself || imageModel.isSend || imageModel.isSending;
    self.progressLabel.hidden  = !imageModel.byMyself || !imageModel.isSending;
    self.nickNameLabel.hidden = imageModel.byMyself;
...
- (void)setContent
{
    ...
    if (_imageModel.byMyself) {
        ...
        //群聊资源缓存路径
        imgCachePath = [ChatCache_Path stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/small_%@",_imageModel.roomId,_imageModel.content.fileName]];
