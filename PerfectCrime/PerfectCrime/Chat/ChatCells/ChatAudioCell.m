...
- (void)setAudioModel:(ChatModel *)audioModel
{
    ...
    //处理失败按钮
    self.failureButton.hidden   = audioModel.isSend || audioModel.isSending || !audioModel.byMyself;
    //昵称隐藏处理
    self.nickNameLabel.hidden = audioModel.byMyself;
    //红点隐藏处理
    self.redPoint.hidden   = YES;// audioModel.byMyself.integerValue || audioModel.isRead.integerValue;
    //转圈处理
    audioModel.isSending &&audioModel.byMyself ? [self.activiView startAnimating] : [self.activiView stopAnimating];
    //赋值
    [self setContent];
    //frame
    [self setFrame];
...
- (void)setContent
{
    ...
    //我方
    if (_audioModel.byMyself) {
        
        //头像
        [self.iconView downloadImage:[[SettingsManager sharedSettingsManager]picUrl] placeholder:@"defaultHead"];
        ...
        if (![CustomStringFunc isBlankString:picFrame]) {
            ...
            photoFrame.image = [UIImage imageNamed:picFrame];
        ...
        //对方头像 TO-DO
        [self.iconView downloadImage:_audioModel.picUrl placeholder:@"defaultHead"];
        if (![CustomStringFunc isBlankString:_audioModel.picFrame]) {
            CGRect frame = self.iconView.frame;
            frame.origin.x = 0;
            frame.origin.y = 0;
            UIImageView *photoFrame = [[UIImageView alloc]initWithFrame:frame];
            photoFrame.backgroundColor = [UIColor clearColor];
            photoFrame.image = [UIImage imageNamed:_audioModel.picFrame];
            [self.iconView addSubview:photoFrame];
        }
        //气泡
...
- (void)playAudio
{
    NSArray *gifs = _audioModel.byMyself ? @[@"我方语音icon01",@"我方语音icon02",@"我方语音icon03"] : @[@"对方语音icon01",@"对方语音icon02",@"对方语音icon03"];
    [self.voiceGIFView GIF_PrePlayWithImageNamesArray:gifs duration:_audioModel.content.seconds.integerValue];
    NSString *audioPath = nil;
    audioPath = [NSString stringWithFormat:@"%@/%@/%@",ChatCache_Path,_audioModel.roomId,_audioModel.content.fileName];
    //回调播放
    if (_playCallback) {
