        ...
        session = [manager GET:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            if ([error code] < 0 && sg_cacheGet) {// 获取缓存
                ...
                if (response) {
                    ...
                    [self handleCallbackWithError:error fail:fail errorTask:task];
...
+ (void)handleCallbackWithError:(NSError *)error fail:(HYBResponseFail)fail errorTask:(NSURLSessionDataTask * _Nonnull )task{
    NSHTTPURLResponse *put_response = (NSHTTPURLResponse *)task.response;
    NSInteger statusCode            = [put_response statusCode];
    if(statusCode == 401){
        [MBProgressHUD hideHUD];
        AppDelegate *appDele =(AppDelegate *)[[UIApplication sharedApplication] delegate];
        [appDele setBeginViewController];
    }else{
        if ([error code] == NSURLErrorCancelled) {
            if (sg_shouldCallbackOnCancelRequest) {
                if (fail) {
